...
router.post("/paciente/:idPaciente", async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: "Faltan datos requeridos: idAlergia y gravedad" });
    }
    ...
    return res
      .status(201)
      .json({
        message: "Alergia añadida al paciente",
        idPaciente,
        idAlergia,
        idSeveridad: gravedad,
      });
  } catch (error) {
    console.error(error);
    ...
    if (error && error.code === "ER_DUP_ENTRY") {
      return res
        .status(409)
        .json({ message: "La alergia ya está registrada para este paciente" });
    }
    return res
      .status(500)
      .json({ message: "Error al insertar alergia del paciente" });
  }
});
